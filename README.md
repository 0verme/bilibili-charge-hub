...
可开源、自托管的多用户 Bilibili 充电记录管理平台。0.4 提供无需操作 Swagger 的完整管理后台，覆盖扫码绑定、增量采集、可选每日任务、任务、通知、分享和用户管理，并统一管理端与公开分享驾驶舱体验。
...
`POST /api/bili/accounts/{id}/collect` 会分页读取该账号的全部充电记录。服务优先使用上游订单标识生成稳定业务键；没有订单标识时，使用账号、支持者 UID、标准化金额/佣金和 UTC 充电时间生成摘要，不把昵称或头像作为身份依据。数据库按账号与业务键去重；首次采集只建立历史基线，水位之后的新记录才进入通知 outbox。每次执行都会保存页数、读取数、新增数、跳过重复数、抑制历史通知数、耗时和错误摘要，同一账号不会并发采集。

账号绑定后默认创建 5 分钟增量采集任务和每日 01:00 B 币券领取任务。任务配置和下次运行时间持久化，立即运行会返回可查询的运行记录。B 币券月份按 `APP_TIMEZONE` 计算；系统不包含自动消费功能。

每日任务（可选，默认关闭）可为账号开启观看、分享、投币赚经验：在账号卡片点击「每日任务」逐项配置，启用后每天 01:30 自动执行，支持目标投币数、保留硬币数、投币时点赞、LV6 跳过、指定 UP 列表和分享/观看开关。投币是消耗硬币的行为，必须在配置页显式启用并二次确认；每次执行都会记录当日结果（投币数、余额、分享/观看状态）供审计。
...
充电记录入库与通知投递分属不同事务：进程、Docker 或数据库短暂中断时，可能出现「充电记录已入库但通知链路静默遗漏」的缺口。系统每小时自动运行一次通知对账（也可由管理员在通知中心手动触发）：

- 扫描最近 24 小时（可通过 `NOTIFICATION_RECONCILIATION_LOOKBACK_HOURS` 调整）入库的充电记录；
- 校验每条记录是否存在对应的 `new_charge` outbox 事件，缺失时用与正常采集完全一致的 payload 和稳定去重键补建；
- 校验已订阅渠道是否都存在投递记录，缺失时补建并交回现有投递链路；
- 已成功投递的通知永不重发；失败通知尊重原有退避与重试预算，不强制重发；
- 全过程幂等、严格按用户隔离，绝不调用 Bilibili API；每次执行输出结构化审计摘要（扫描数 / 补建数 / 补齐投递数 / 重入队数等）。
...
- 每日任务（含观看/分享/投币）默认关闭，必须逐账号显式启用；启用后分享默认开启、观看默认关闭，投币按目标投币数执行并受保留硬币数、LV6 跳过与当日去重保护，保存前会二次确认。
